...
class CommandRouter:
    ...
    async def handle_text(
        ...
        # Si un outil a RAPPORTÉ une réponse (recherche web, programme TV...), c'est
        # ELLE que Jarvis prononce — le LLM planificateur ne connaît pas l'info, seul
        # l'outil la ramène. On prend la 1re réponse d'outil contenant un champ "answer".
        tool_answer = next(
            (
                e.response.get("answer")
                for e in executions
                if e.response.get("answer")
            ),
            None,
        )
        speak = tool_answer or plan.spoken_response or self._fallback_speak(rejection_reason)
        return CommandRouterResult(
            speak=speak,
